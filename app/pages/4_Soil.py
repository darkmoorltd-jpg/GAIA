...
SOIL_COLORS = {
    "Alluvial": "#8d6e63", "Arid": "#d4a373", "Black": "#3e2723",
    "Laterite": "#b7410e", "Mountain": "#78909c", "Red": "#c62828", "Yellow": "#f9a825"
}
...
if theme == "dark":
    ...
    <style>
        ...
        .soil-particle { position: fixed; border-radius: 50%; background: radial-gradient(circle, rgba(139,119,90,0.4), transparent); animation: soilFloat 8s infinite ease-in-out; z-index: 0; pointer-events: none; }
        ...
        @keyframes soilFloat { 0% { transform: translateY(0px) scale(1); opacity: 0.3; } 50% { transform: translateY(-40px) scale(1.2); opacity: 0.6; } 100% { transform: translateY(0px) scale(1); opacity: 0.3; } }
        .scan-ring { position: absolute; top: 50%; left: 50%; width: 300px; height: 300px; border-radius: 50%; border: 3px solid rgba(212,163,115,0.6); transform: translate(-50%, -50%); animation: scanPulse 2s infinite ease-out; z-index: 2; pointer-events: none; }
        .scan-ring:nth-child(2) { animation-delay: 0.7s; } .scan-ring:nth-child(3) { animation-delay: 1.4s; }
        @keyframes scanPulse { 0% { width: 100px; height: 100px; opacity: 1; } 100% { width: 400px; height: 400px; opacity: 0; } }
        .title { font-size: 3.5rem; font-weight: 900; text-align: center; background: linear-gradient(90deg, #d4a373, #f5e6d3, #d4a373); -webkit-background-clip: text; -webkit-text-fill-color: transparent; text-shadow: 0 0 25px rgba(212,163,115,0.7); margin-bottom: 0.5rem; position: relative; z-index: 1; animation: soilGlow 2s ease-in-out infinite alternate; }
        ...
        .stFileUploader > div { background: rgba(255,255,255,0.05) !important; backdrop-filter: blur(12px) !important; border: 2px dashed rgba(212,163,115,0.4) !important; border-radius: 20px !important; padding: 2rem !important; transition: all 0.3s ease; }
        .stFileUploader > div:hover { border-color: #d4a373 !important; background: rgba(212,163,115,0.1) !important; }
        .stImage img { border-radius: 20px; box-shadow: 0 0 40px rgba(212,163,115,0.3); }
        .soil-result-card { background: rgba(255,255,255,0.05); backdrop-filter: blur(20px); border: 1px solid rgba(212,163,115,0.2); border-radius: 20px; padding: 1.5rem; margin: 0.5rem 0; transition: all 0.3s ease; position: relative; overflow: hidden; }
        .soil-result-card::before { content: ''; position: absolute; top: 0; left: 0; width: 5px; height: 100%; border-radius: 3px; }
        ...
        .action-box { background: rgba(212,163,115,0.15); border: 1px solid rgba(212,163,115,0.3); border-radius: 15px; padding: 1.5rem; text-align: center; margin-top: 1rem; backdrop-filter: blur(10px); }
    </style>
    <div class="soil-particle"></div><div class="soil-particle"></div><div class="soil-particle"></div><div class="soil-particle"></div><div class="soil-particle"></div>
    """, unsafe_allow_html=True)
else:
    st.markdown("""
    <style>
        .stApp { background: linear-gradient(135deg, #efebe9 0%, #d7ccc8 100%); color: #3e2723; }
        .soil-particle { display: none; } .scan-ring { display: none; }
        .title { font-size: 3.5rem; font-weight: 900; text-align: center; background: linear-gradient(90deg, #5d4037, #8d6e63, #5d4037); -webkit-background-clip: text; -webkit-text-fill-color: transparent; text-shadow: 0 0 10px rgba(93,64,55,0.3); margin-bottom: 0.5rem; animation: soilGlowLight 2s ease-in-out infinite alternate; }
        @keyframes soilGlowLight { from { text-shadow: 0 0 10px rgba(93,64,55,0.3); } to { text-shadow: 0 0 25px rgba(93,64,55,0.8), 0 0 50px rgba(93,64,55,0.5); } }
        ...
        .stFileUploader > div:hover { border-color: #5d4037 !important; background: rgba(141,110,99,0.1) !important; }
        .stImage img { border-radius: 20px; box-shadow: 0 0 20px rgba(0,0,0,0.2); }
        .soil-result-card { background: rgba(255,255,255,0.8); backdrop-filter: blur(10px); border: 1px solid rgba(0,0,0,0.1); border-radius: 20px; padding: 1.5rem; margin: 0.5rem 0; transition: all 0.3s ease; }
        ...
        .action-box { background: rgba(141,110,99,0.1); border: 1px solid rgba(93,64,55,0.2); border-radius: 15px; padding: 1.5rem; text-align: center; margin-top: 1rem; }
    </style>
    """, unsafe_allow_html=True)
...
if uploaded_file:
    ...
    with col2:
        ...
        if theme == "dark":
            st.markdown('<div style="position:relative;height:0;"><div class="scan-ring"></div><div class="scan-ring"></div><div class="scan-ring"></div></div>', unsafe_allow_html=True)
    st.markdown("---")
    ...
    <div class="soil-result-card top-result" style="border-left: 5px solid {top_color};">
        <h2 style="margin:0; display: flex; align-items: center;">
            <span class="soil-swatch" style="background-color: {top_color};"></span>{top_soil}
