...
has_saved_profile = bool(profile and profile.get(
    "first_name") and profile.get("last_name"))
...
with st.form("complete_profile_form"):
    ...
    with col1:
        ...
        gender = st.selectbox(
            "Gender", [
                "", "Male", "Female"], disabled=not can_edit)
    with col2:
        ...
        marital_status = st.selectbox(
            "Marital Status", [
                "", "Single", "Married", "Divorced", "Widowed"], disabled=not can_edit)
    with col3:
        phone = st.text_input(
    ...
    with col1:
        country = st.text_input(
            ...
            value=profile.get(
                "country",
                "Nigeria") if profile else "Nigeria",
            disabled=not can_edit)
        state = st.text_input(
            "State *",
            value=profile.get(
    ...
    with col1:
        ...
        nin = st.text_input(
            ...
            max_chars=11,
            disabled=not can_edit)
        govt_id_type = st.selectbox("Government ID Type",
                                    ["",
                                     "National ID Card",
                                     "Driver's License",
    ...
    with col1:
        notify_sms = st.checkbox(
            "SMS Notifications",
            value=True if not profile else profile.get(
                "notify_sms",
                True),
            disabled=not can_edit)
        notify_whatsapp = st.checkbox(
            "WhatsApp", value=True if not profile else profile.get(
                "notify_whatsapp", True), disabled=not can_edit)
        notify_weather = st.checkbox(
            "Weather Alerts", value=True if not profile else profile.get(
                "notify_weather", True), disabled=not can_edit)
    with col2:
        notify_disease = st.checkbox(
            "Disease Alerts", value=True if not profile else profile.get(
                "notify_disease", True), disabled=not can_edit)
        notify_payment = st.checkbox(
            "Payment Alerts", value=True if not profile else profile.get(
                "notify_payment", True), disabled=not can_edit)
        preferred_language = st.selectbox(
            "Language", [
                "English", "Hausa", "Yoruba", "Igbo", "Pidgin English"], disabled=not can_edit)
        ...
        try:
            if profile:
                ...
                service.table("user_profiles").update(
                    update_data).eq("user_id", user.id).execute()
            else:
                # Insert new
