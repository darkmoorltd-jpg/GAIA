...
st.set_page_config(
    page_title="GAIA – Verification History", page_icon="📋", layout="wide"
)
...
st.markdown(
    "<style>.stApp{background:linear-gradient(135deg,#f5f7fa,#e8f5e9)}.title{font-size:2.5rem;font-weight:800;text-align:center;color:#2e7d32}</style>",
    unsafe_allow_html=True,
)
st.markdown(
    '<div class="title">📋 Verification & Payment History</div>', unsafe_allow_html=True
)

history = (
    supabase.table("farmer_verifications")
    .select("*")
    .eq("user_id", user.id)
...
if history.data and len(history.data) > 0:
    for h in history.data:
        ...
        with st.expander(
            f"{emoji} {status.upper()} — {str(h.get('created_at', ''))[:16]}"
        ):
