...
st.markdown("""
<style>
    .top-nav { display: flex; justify-content: center; gap: 2rem; padding: 0.8rem; background: rgba(255,255,255,0.9); backdrop-filter: blur(15px); border-radius: 15px; margin-bottom: 2rem; box-shadow: 0 4px 15px rgba(0,0,0,0.05); }
...
""", unsafe_allow_html=True)
dark_mode = st.toggle("", value=(st.session_state.theme == "dark"), key="crops_theme")
...
# ---------- Crop definitions & models (same as before, kept compact for space) ----------
CROP_CLASSES = {
    ...
    "tomato": ["Bacterial_spot","Early_blight","Late_blight","Leaf_Mold","Septoria_leaf_spot","Spider_mites Two-spotted_spider_mite","Target_Spot","Tomato_Yellow_Leaf_Curl_Virus","Tomato_mosaic_virus","healthy","powdery_mildew"],
...
def load_crop_model(crop_name):
    if crop_name in ["apple","mango","tomato","orange"]:
        ...
        model = AppleViT13() if crop_name=="apple" else (MangoViT8() if crop_name=="mango" else (TomatoViT11() if crop_name=="tomato" else OrangeViT5()))
...
if uploaded_files:
    for uploaded_file in uploaded_files:
        ...
        with col2:
            try:
                model = load_crop_model(crop); probs = predict_image(model, image)
                class_names = CROP_CLASSES[crop]; top_idx = np.argmax(probs)
                st.markdown(f'<div class="result-card top-result"><h3 style="margin:0;">{class_names[top_idx]}</h3><span style="font-size:1.5rem;color:#00ff88;">{probs[top_idx]*100:.1f}%</span></div>', unsafe_allow_html=True)
