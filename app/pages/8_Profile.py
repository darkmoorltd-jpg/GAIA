...
if profile_locked:
    st.markdown(
        '<div class="locked-banner">🔒 Profile locked. Contact darkmoorltd@gmail.com to edit.</div>',
        unsafe_allow_html=True,
    )
...
with st.form("complete_profile_form"):
    ...
    with col1:
        first_name = st.text_input(
            "First Name *",
            value=profile.get("first_name", "") if profile else "",
            disabled=not can_edit,
        )
        middle_name = st.text_input(
            "Middle Name",
            value=profile.get("middle_name", "") if profile else "",
            disabled=not can_edit,
        )
    ...
    with col2:
        last_name = st.text_input(
            "Last Name *",
            value=profile.get("last_name", "") if profile else "",
            disabled=not can_edit,
        )
        date_of_birth = st.date_input(
            "Date of Birth", value=None, disabled=not can_edit, key="dob"
        )
    ...
    with col3:
        phone = st.text_input(
            "Phone * (for SMS receipts)",
            value=profile.get("phone", "") if profile else "",
            disabled=not can_edit,
        )
        whatsapp = st.text_input(
            "WhatsApp *",
            value=profile.get("whatsapp", "") if profile else "",
            disabled=not can_edit,
        )
    ...
    with col1:
        country = st.text_input(
            "Country *",
            value=profile.get("country", "Nigeria") if profile else "Nigeria",
            disabled=not can_edit,
        )
        state = st.text_input(
            "State *",
            value=profile.get("state", "") if profile else "",
            disabled=not can_edit,
        )
        lga = st.text_input(
            "LGA",
            value=profile.get("lga", "") if profile else "",
            disabled=not can_edit,
        )
    with col2:
        city = st.text_input(
            "City/Town *",
            value=profile.get("city", "") if profile else "",
            disabled=not can_edit,
        )
        street_address = st.text_input(
            "Street Address",
            value=profile.get("street_address", "") if profile else "",
            disabled=not can_edit,
        )
        landmark = st.text_input(
            "Landmark",
            value=profile.get("landmark", "") if profile else "",
            disabled=not can_edit,
        )
    with col3:
        postal_code = st.text_input(
            "Postal Code",
            value=profile.get("postal_code", "") if profile else "",
            disabled=not can_edit,
        )
    ...
    with col1:
        bvn = st.text_input(
            "BVN (11 digits)",
            value=profile.get("bvn", "") if profile else "",
            max_chars=11,
            disabled=not can_edit,
        )
        nin = st.text_input(
            "NIN (11 digits)",
            value=profile.get("nin", "") if profile else "",
            max_chars=11,
            disabled=not can_edit,
        )
    ...
    with col2:
        govt_id_number = st.text_input(
            "ID Number",
            value=profile.get("govt_id_number", "") if profile else "",
            disabled=not can_edit,
        )
    ...
    col1, col2, col3 = st.columns(3)
    with col1:
        farm_state = st.text_input(
            "Farm State",
            value=profile.get("farm_state", "") if profile else "",
            disabled=not can_edit,
        )
        farm_size = st.number_input(
            "Farm Size (acres)",
            min_value=0.0,
            value=float(profile.get("farm_size_acres", 1.0) if profile else 1.0),
            disabled=not can_edit,
        )
    ...
    with col2:
        farm_lga = st.text_input(
            "Farm LGA",
            value=profile.get("farm_lga", "") if profile else "",
            disabled=not can_edit,
        )
        years_exp = st.number_input(
            "Years of Experience",
            min_value=0,
            value=int(profile.get("years_experience", 0) if profile else 0),
            disabled=not can_edit,
        )
        primary_crops = st.text_input(
            "Primary Crops",
            value=profile.get("primary_crops", "") if profile else "",
            disabled=not can_edit,
        )
    with col3:
        farm_address = st.text_input(
            "Farm Address",
            value=profile.get("farm_address", "") if profile else "",
            disabled=not can_edit,
        )
    ...
    with col1:
        account_name = st.text_input(
            "Account Name",
            value=profile.get("account_name", "") if profile else "",
            disabled=not can_edit,
        )
    ...
    with col2:
        account_number = st.text_input(
            "Account Number",
            value=profile.get("account_number", "") if profile else "",
            max_chars=10,
            disabled=not can_edit,
        )
    ...
    with col1:
        emergency_name = st.text_input(
            "Contact Name",
            value=profile.get("emergency_contact_name", "") if profile else "",
            disabled=not can_edit,
        )
        emergency_relationship = st.text_input(
            "Relationship",
            value=profile.get("emergency_relationship", "") if profile else "",
            disabled=not can_edit,
        )
    with col2:
        emergency_phone = st.text_input(
            "Contact Phone",
            value=profile.get("emergency_contact_phone", "") if profile else "",
            disabled=not can_edit,
        )
    ...
    col1, col2 = st.columns(2)
    with col1:
        notify_sms = st.checkbox(
            "SMS Notifications",
            value=True if not profile else profile.get("notify_sms", True),
            disabled=not can_edit,
        )
        notify_whatsapp = st.checkbox(
            "WhatsApp",
            value=True if not profile else profile.get("notify_whatsapp", True),
            disabled=not can_edit,
        )
        notify_weather = st.checkbox(
            "Weather Alerts",
            value=True if not profile else profile.get("notify_weather", True),
            disabled=not can_edit,
        )
    with col2:
        notify_disease = st.checkbox(
            "Disease Alerts",
            value=True if not profile else profile.get("notify_disease", True),
            disabled=not can_edit,
        )
        notify_payment = st.checkbox(
            "Payment Alerts",
            value=True if not profile else profile.get("notify_payment", True),
            disabled=not can_edit,
        )
    ...
    if can_edit:
        submitted = st.form_submit_button(
            "💾 Save Profile & Lock", type="primary", use_container_width=True
        )
    else:
    ...
    if not first_name or not last_name or not phone:
        ...
        update_data = {
            ...
            "emergency_contact_name": (
                emergency_name.strip() if emergency_name else None
            ),
            "emergency_contact_phone": (
                emergency_phone.strip() if emergency_phone else None
            ),
            "emergency_relationship": (
                emergency_relationship.strip() if emergency_relationship else None
            ),
        ...
        try:
            if profile:
                # Update existing
                service.table("user_profiles").update(update_data).eq(
                    "user_id", user.id
                ).execute()
            else:
                # Insert new
            ...
            st.success(
                "✅ Profile saved! Phone number is now ready for Paystack SMS receipts."
            )
            st.balloons()
            st.rerun()
