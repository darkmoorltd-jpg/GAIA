...
def stream_deepseek_livestock_guide(disease, animal, confidence):
    prompt = f"""GAIA diagnosed: {disease} in {animal} with {
...
if files:
    ...
    for f in files:
        ...
        with st.expander(f"🐄 {f.name}", expanded=True):
            ...
            if col_fb1.button("👍 Helpful", key=f"livestock_help_{f.name}"):
                save_feedback(f.name, td, True)
                col_fb1.success("Thanks!")
            if col_fb2.button("👎 Not", key=f"livestock_not_{f.name}"):
               save_feedback(f.name, td, False)
