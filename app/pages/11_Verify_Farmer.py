...
st.set_page_config(
    page_title="GAIA – Farmer Verification", page_icon="🛡️", layout="wide"
)
...
st.markdown(
    '<div class="subtitle">Verify your identity to unlock wallet, insurance, and marketplace features</div>',
    unsafe_allow_html=True,
)
...
try:
    existing = (
        service.table("farmer_verifications")
        .select("*")
        .eq("user_id", user.id)
        .execute()
    )
...
with st.form("verification_form"):
    ...
    with col1:
        ...
        phone = st.text_input(
            "Phone Number *", value=user_phone, placeholder="e.g., 08031234567"
        )
    ...
    with col2:
        ...
        selfie_upload = st.file_uploader(
            "Upload Selfie with ID *", type=["jpg", "jpeg", "png"]
        )
    ...
    submit = st.form_submit_button(
        "✅ Submit for Verification", type="primary", use_container_width=True
    )
