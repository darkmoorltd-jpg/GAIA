...
st.markdown('<div class="subtitle">Your personalized season‑long action plan</div>', unsafe_allow_html=True)

# ---------- Input Form ----------
col1, col2 = st.columns(2)
with col1:
    crop = st.selectbox("🌾 Select Crop", list(CROP_TEMPLATES.keys()))
with col2:
...
if st.button("Generate My Calendar", type="primary"):
    ...
    template = CROP_TEMPLATES[crop]

    # Build activities with actual dates
    activities = []
    ...
    # Optional: Use DeepSeek to add a personalized tip
    try:
        import requests
        prompt = f"Based on {crop} farming in {location or 'Nigeria'}, give one personalized tip for the week after planting. Keep it under 100 words."
        headers = {"Authorization": f"Bearer {DEEPSEEK_API_KEY}", "Content-Type": "application/json"}
        payload = {
            "model": "deepseek-chat",
            "messages": [{"role": "system", "content": "You are GAIA, an expert agricultural assistant."},
        ...
        if resp.status_code == 200:
            ...
            activities.append({"week": 1, "date": (planting_date + datetime.timedelta(weeks=1)).isoformat(),
                               "activity": "💡 AI Tip: " + tip, "type": "tip"})
    except:
        pass
    ...
    for act in activities:
        week = act["week"]
        date_obj = datetime.date.fromisoformat(act["date"])
        date_str = date_obj.strftime('%d %b %Y')
        act_type = act["type"]
        act_text = act["activity"]
    ...
    if res.data:
        for cal in res.data:
            with st.expander(f"🌾 {cal['crop']} — planted {cal['planting_date']}"):
                ...
                for act in activities:
                    st.write(f"Week {act['week']} ({act['date']}): **{act['activity']}**")
                if cal.get("location"):
