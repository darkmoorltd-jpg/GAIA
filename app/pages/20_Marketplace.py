...
with tab1:
    ...
    for row_idx, row in enumerate(rows):
        ...
        for i, listing in enumerate(row):
            with cols[i]:
                render_card(listing, row_idx * 3 + i + 100)  # Offset to avoid key conflicts
...
with tab2:
    ...
    if not has_seller_profile:
        st.warning("🔐 Complete your seller registration first before listing produce.")
        ...
        with st.form("seller_registration_form"):
            ...
            with col3:
                primary_crops = st.text_input("🌾 Primary Crops", placeholder="e.g., Maize, Rice")
            
            submit_profile = st.form_submit_button("✅ Register as Seller", type="primary", use_container_width=True)
            ...
            if submit_profile:
                if not full_name or not phone or not whatsapp or not address or not state:
                    ...
                    try:
                        ...
                        st.success("✅ Seller registration complete!")
                        st.balloons()
                        st.rerun()
                    except Exception as e:
                        st.error(f"Error: {str(e)[:200]}")
    else:
        st.success(f"✅ Registered: **{seller_profile.get('full_name','')}** | 📞 {seller_profile.get('phone','')} | 💬 {seller_profile.get('whatsapp','')}")
        ...
        with st.form("sell_produce_form"):
            ...
            submit_listing = st.form_submit_button("📤 List My Produce", type="primary", use_container_width=True)
...
with tab3:
    ...
    if not store:
        st.info("No listings yet. Go to Sell Produce.")
    else:
...
with cols[5]: st.page_link("pages/17_Video_Scan.py", label="🎥 Video")
...
with cols[8]: st.page_link("pages/16_Chat.py", label="💬 Chat")
with cols[9]: st.page_link("pages/9_Buy_Scans.py", label="💳 Buy Scans")
