...
def load_crop_model(crop_name: str):
    """Load the correct model for the given crop based on its config."""
    ...
    # Load config if it exists, otherwise fall back to guessing num_classes
    if os.path.exists(config_path):
        ...
        # Fallback: derive from the class list
        num_classes = len(CROP_CLASSES[crop_name])
        model_type = "vit_pretrained"   # all recent crops use pretrained ViT
        in_chans = 3
...
# ---------- Inference helper ----------
def predict_image(model, image: Image.Image, num_classes: int):
...
if uploaded_file is not None:
    ...
    try:
        ...
        probs = predict_image(model, image, num_classes)
