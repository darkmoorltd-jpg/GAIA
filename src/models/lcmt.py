...
class MultiScaleFeaturePyramid(nn.Module):
    def __init__(self, embed_dim=384):
        super().__init__()
        self.scale1 = timm.create_model(
            'vit_small_patch16_224',
            pretrained=False,
            num_classes=0)
        self.scale2 = timm.create_model(
            'vit_small_patch16_224',
            pretrained=False,
            num_classes=0)
        self.scale3 = timm.create_model(
            'vit_small_patch16_224',
            pretrained=False,
            num_classes=0)
...
class LesionDetector(nn.Module):
    def __init__(self, embed_dim=384):
        ...
        self.lesion_detector = nn.Sequential(
            nn.Linear(
                embed_dim, 256), nn.GELU(), nn.Linear(
                256, 1))
        self.lesion_counter = nn.Sequential(
            nn.Linear(
                embed_dim, 128), nn.GELU(), nn.Linear(
                128, 1))
    ...
    def forward(self, patch_tokens):
        ...
        return lesion_global, lesion_probs.squeeze(
            -1), lesion_count.squeeze(-1)
...
class LCMT(nn.Module):
    ...
    def forward(self, x):
        ...
        lesion_global, lesion_probs, lesion_count = self.lesion_detector(
            patch_tokens)
