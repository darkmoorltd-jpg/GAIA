...
if user is None:
    ...
    res = service.table("user_scans").select("scans_remaining, plan").eq("user_id", (user.id if user is not None else None)).execute()
...
user_phone = ""
try:
    profile_res = service.table("user_profiles").select("phone").eq("user_id", (user.id if user is not None else None)).execute()
...
if st.session_state.selected_plan:
    ...
    ref = f"GAIA_{(user.id if user is not None else None)[:8]}_{st.session_state.selected_plan}_{uuid.uuid4().hex[:6]}"
            ...
            if v["ok"]:
                ...
                if exist.data:
                    ...
                    if match:
                        ...
                        cur = db.table("user_scans").select("scans_remaining").eq("user_id", (user.id if user is not None else None)).execute()
                        cur_scans = cur.data[0]["scans_remaining"] if cur.data else 0
                        new_total = cur_scans + add
                        db.table("user_scans").update({"scans_remaining": new_total, "plan": match}).eq("user_id", (user.id if user is not None else None)).execute()
                        db.table("payment_history").insert({"user_id": (user.id if user is not None else None), "amount": amt, "scans_added": add, "plan": match, "reference": ref_input}).execute()
