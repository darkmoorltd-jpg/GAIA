...
with tab_table:
    if df.empty:
        ...
        search = st.text_input("Search by name, phone, state, crop", placeholder="e.g., Ibrahim, 0803, Kano, Maize")
        filtered = df
        if search:
            q = search.lower()
            filtered = df[
        ...
        st.dataframe(filtered[["full_name", "phone", "state", "lga", "crop", "farm_size_acres", "verification_status"]],
                     use_container_width=True, hide_index=True)
        st.download_button("📥 Export CSV", filtered.to_csv(index=False), "farmers.csv", "text/csv")
