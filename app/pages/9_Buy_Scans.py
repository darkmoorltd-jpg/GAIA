...
<style>
    ...
    .plan-save { font-size: 0.8rem; color: #4caf50; font-weight: 600; }
    .big-pay-btn {
        background: linear-gradient(135deg, #2e7d32, #4caf50);
        color: #fff; border: none; padding: 20px 40px;
        border-radius: 40px; font-weight: 700; font-size: 1.3rem;
        cursor: pointer; width: 100%; margin-top: 1rem;
        box-shadow: 0 8px 25px rgba(46,125,50,.3);
    }
    .big-pay-btn:hover { transform: scale(1.03); box-shadow: 0 12px 35px rgba(46,125,50,.4); }
    .selected-banner {
...
if st.session_state.chosen_plan:
    ...
    <div class="selected-banner">
        <h3>🛒 {plan_data['scans']} scans — {plan_data['price']}</h3>
    </div>
    """, unsafe_allow_html=True)
    
    ref = f"GAIA_{user.id[:8]}_{st.session_state.chosen_plan}_{uuid.uuid4().hex[:6]}"
    
    # ONE single big Paystack button - opens full screen
    paystack_html = f"""
    <!DOCTYPE html>
    <html>
    <head>
        ...
        <style>
            ...
            .pay-btn:hover {{ transform: scale(1.05); box-shadow: 0 12px 35px rgba(46,125,50,.4); }}
        </style>
    </head>
    <body>
