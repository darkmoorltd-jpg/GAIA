...
def upload_file_to_supabase(file_bytes, filename):
    ...
    try:
        ...
        try:
            ...
            supabase.storage.from_("support_attachments").upload(clean_name, file_bytes)
            url = supabase.storage.from_("support_attachments").get_public_url(
                clean_name
            )
            return url, None
...
with tab1:
    ...
    with st.form("new_ticket_form"):
        subject = st.text_input(
            "Subject *", placeholder="e.g., Payment issue, Crop question"
        )
        message = st.text_area(
            "Your Message *", placeholder="Describe your issue...", height=150
        )
        uploaded_file = st.file_uploader(
            "📎 Attach Image/File",
            type=["jpg", "jpeg", "png", "gif", "mp4", "mp3", "wav", "pdf"],
        )
        ...
        if st.form_submit_button("📤 Send", type="primary", use_container_width=True):
            if not subject or not message:
                ...
                if uploaded_file:
                    ...
                    attachment_url, err = upload_file_to_supabase(
...
with tab2:
    ...
    if not my_tickets:
        ...
        for ticket in my_tickets:
            ...
            with st.expander(
                f"{emoji} {ticket.get('subject', '')} — {str(ticket.get('created_at', ''))[:16]}",
                expanded=True,
            ):
                st.markdown(f"**You wrote:** {ticket.get('message', '')}")

                if ticket.get("attachment_url"):
                    st.markdown(f"[📎 Download Attachment]({
                ...
                if reply_list:
                    for reply in reply_list:
                        if reply.get("is_admin"):
                            st.markdown(
                                f'<div class="msg-admin"><strong>🔐 GAIA Team:</strong><br>{
                                    reply.get(
                                        "message",
                                        "")}</div>',
                                unsafe_allow_html=True,
                            ...
                            st.markdown(
                                f'<div class="msg-user"><strong>👤 You:</strong><br>{
                                    reply.get(
                                        "message",
                ...
                with st.form(f"user_reply_form_{ticket_id}"):
                    user_reply = st.text_area(
                        "Add a reply", key=f"user_reply_text_{ticket_id}", height=60
                    )
