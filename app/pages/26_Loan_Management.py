...
with tab1:
    with st.form("loan_form"):
        ...
        with col1:
            ...
            duration = st.number_input("Duration (months)", min_value=3, max_value=24, value=6)
        with col2:
            farm_location = st.text_input("Farm Location (LGA/State)")
            purpose = st.text_area("Purpose of Loan")
            interest_rate = st.number_input("Interest Rate (% annual)", min_value=0.0, max_value=30.0, value=12.0)
...
with tab2:
    ...
    if loans:
        ...
        total_disbursed = df[df["status"] == "approved"]["loan_amount"].astype(float).sum()
...
with tab3:
    repayments_res = supabase.table("loan_repayments").select("*").order("paid_at", desc=True).execute()
