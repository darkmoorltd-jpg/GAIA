...
with cols[5]: st.page_link("pages/9_Buy_Scans.py", label="💳 Buy Scans")    paystack_html = f"""<!DOCTYPE html>
<html>
<head><script src="https://js.paystack.co/v1/inline.js"></script></head>
<body style="margin:0;display:flex;justify-content:center;">
    <button onclick="payWithPaystack()" style="padding:18px 50px;background:linear-gradient(135deg,#2e7d32,#43a047);color:#fff;border:none;border-radius:50px;font-weight:700;font-size:1.2rem;cursor:pointer;box-shadow:0 10px 30px rgba(46,125,50,0.3);">💳 Pay {plan_data['price']} Now</button>
    <script>
        function payWithPaystack(){{
            PaystackPop.setup({{
                key:'{PAYSTACK_PUBLIC_KEY}',
                ...
                label:'GAIA {scans_display}',
                onClose:function(){{window.parent.location.reload();}},
                callback:function(response){{window.location.href='https://gaiagpt.streamlit.app/~/callback?reference='+response.reference+'&plan={st.session_state.chosen_plan}';}}
            }}).openIframe();
        }}
    </script>
</body>
</html>"""
    components.html(paystack_html, height=100)nents.v1 as components
from supabase import create_client, Client
import uuid
import requests as req
