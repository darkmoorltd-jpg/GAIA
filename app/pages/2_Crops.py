...
def load_crop_model(crop_name):
    ...
    head_weight = state.get("head.weight", state.get(f"{prefix}head.weight"))
    head_bias = state.get("head.bias", state.get(f"{prefix}head.bias"))
    if head_weight is not None:
        head.load_state_dict({"weight": head_weight, "bias": head_bias if head_bias is not None else torch.zeros(n)}, strict=False)
