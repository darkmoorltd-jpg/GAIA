...
if theme == "dark":
    st.markdown(f"""
    <style>
        .stApp {{ background: linear-gradient(135deg, #0a0a0a 0%, #1a1a2e 100%); color: #e0e0e0; }}
        header, footer {{visibility: hidden;}}
        .bg-container {{ position: fixed; top: 0; left: 0; width: 100%; height: 100%; z-index: -2; background: url('{bg_image}') center/cover no-repeat fixed; }}
        .bg-overlay {{ position: fixed; top: 0; left: 0; width: 100%; height: 100%; z-index: -1; background: {bg_overlay}; }}
        .title {{ font-size: 3rem; font-weight: 900; text-align: center; background: linear-gradient(90deg, {accent_color}, #fff, {accent_color}); -webkit-background-clip: text; -webkit-text-fill-color: transparent; text-shadow: 0 0 30px {glow_color}; animation: titleGlow 2s ease-in-out infinite alternate; position: relative; z-index: 1; }}
        @keyframes titleGlow {{ from {{ text-shadow: 0 0 30px {glow_color}; }} to {{ text-shadow: 0 0 60px {glow_color}, 0 0 100px {glow_color}; }} }}
        .subtitle {{ text-align: center; font-size: 1.1rem; color: #ddd; margin-bottom: 2rem; position: relative; z-index: 1; }}
        .stFileUploader > div {{ background: rgba(255,255,255,0.03) !important; backdrop-filter: blur(15px) !important; border: 2px dashed rgba({accent_color.replace('#','')}, 0.3) !important; border-radius: 20px !important; padding: 2rem !important; position: relative; z-index: 1; }}
        .stFileUploader > div:hover {{ border-color: {accent_color} !important; box-shadow: 0 0 30px {glow_color}; }}
        .stImage img {{ border-radius: 20px; box-shadow: 0 0 40px {glow_color}; border: 1px solid {accent_color}; position: relative; z-index: 1; }}
        .result-card {{ background: rgba(0,0,0,0.6); backdrop-filter: blur(25px); border: 1px solid {accent_color}44; border-radius: 20px; padding: 1.5rem; margin: 0.8rem 0; position: relative; overflow: hidden; }}
        .result-card::before {{ content: ''; position: absolute; top: -50%; left: -50%; width: 200%; height: 200%; background: conic-gradient(transparent, {accent_color}22, transparent, transparent); animation: rotate 6s linear infinite; }}
        @keyframes rotate {{ from {{ transform: rotate(0deg); }} to {{ transform: rotate(360deg); }} }}
        .result-card > * {{ position: relative; z-index: 1; }}
        .top-result {{ background: rgba(0,0,0,0.8); border: 2px solid {accent_color}; box-shadow: 0 0 50px {glow_color}, inset 0 0 30px {accent_color}11; }}
        .top-result h3 {{ font-size: 1.2rem; text-transform: uppercase; letter-spacing: 2px; color: {accent_color}; margin: 0.3rem 0; }}
        .counter {{ font-size: 2rem; font-weight: 900; color: {accent_color}; text-shadow: 0 0 30px {glow_color}; animation: pulse 2s ease-in-out infinite; }}
        ...
        .progress-fill {{ height: 100%; border-radius: 8px; background: linear-gradient(90deg, {accent_color}, #fff, {accent_color}); background-size: 200% 100%; animation: shimmer 2s ease infinite, grow 1.5s ease-out; box-shadow: 0 0 10px {glow_color}; transition: width 1s ease; }}
        @keyframes shimmer {{ 0% {{ background-position: 200% 0; }} 100% {{ background-position: -200% 0; }} }}
        @keyframes grow {{ from {{ width: 0% !important; }} }}
        .action-box {{ background: {accent_color}11; border: 1px solid {accent_color}33; border-radius: 15px; padding: 1rem; text-align: center; margin-top: 1rem; backdrop-filter: blur(10px); }}
    </style>
    ...
    <style>
        ...
        .title {{ font-size: 3rem; font-weight: 900; text-align: center; background: linear-gradient(90deg, {accent_color}, #333, {accent_color}); -webkit-background-clip: text; -webkit-text-fill-color: transparent; text-shadow: 0 0 10px {glow_color}; animation: titleGlowLight 2s ease-in-out infinite alternate; }}
        @keyframes titleGlowLight {{ from {{ text-shadow: 0 0 10px {glow_color}; }} to {{ text-shadow: 0 0 25px {glow_color}, 0 0 50px {glow_color}; }} }}
        .subtitle {{ text-align: center; font-size: 1.1rem; color: #333; margin-bottom: 2rem; }}
        .stFileUploader > div {{ background: rgba(255,255,255,0.8) !important; backdrop-filter: blur(10px) !important; border: 2px dashed {accent_color}66 !important; border-radius: 20px !important; padding: 2rem !important; }}
        ...
        .progress-fill {{ height: 100%; border-radius: 8px; background: linear-gradient(90deg, {accent_color}, #fff); animation: grow 1.5s ease-out; }}
        @keyframes grow {{ from {{ width: 0% !important; }} }}
        .action-box {{ background: {accent_color}11; border: 1px solid {accent_color}33; border-radius: 15px; padding: 1rem; text-align: center; margin-top: 1rem; }}
    </style>
    """, unsafe_allow_html=True)
