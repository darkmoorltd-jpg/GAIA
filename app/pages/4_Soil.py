...
st.markdown(
    '<div class="title">🏞️ Soil Type Analysis</div>',
    unsafe_allow_html=True)
st.markdown(
...
if files:
    ...
    for f in files:
        ...
        with st.expander(f"🏞️ {f.name}", expanded=True):
            ...
            with torch.no_grad():
                probs = F.softmax(model(transform(img).unsqueeze(0)), dim=1)[
                    0].detach().cpu().numpy()
    ...
    with st.spinner("🧠 GAIA is preparing your soil management guide..."):
        with st.expander("📋 Complete Soil Management Guide (AI-Generated)", expanded=True):
            ...
            def local_generator():
                for chunk in stream_deepseek_soil_guide(
                        consensus_name, confidence):
