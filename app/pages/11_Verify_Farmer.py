...
st.set_page_config(
    page_title="GAIA – Farmer Verification",
    page_icon="🛡️",
    layout="wide")
...
st.markdown(
    '<div class="title">🛡️ Farmer Verification</div>',
    unsafe_allow_html=True)
st.markdown(
...
try:
    existing = service.table("farmer_verifications").select(
...
with st.form("verification_form"):
    ...
    with col1:
        full_name = st.text_input(
            "Full Name *", placeholder="e.g., Ibrahim Musa")
        phone = st.text_input(
            "Phone Number *",
            value=user_phone,
    ...
    with col2:
        ...
        address = st.text_input(
            "Address *", placeholder="e.g., 12 Main Street")
        crops = st.text_input(
            "Primary Crops",
    ...
    with col1:
        ...
        id_number = st.text_input(
            "ID Number *", placeholder="e.g., 12345678901")
    with col2:
        id_upload = st.file_uploader(
            "Upload ID *",
            type=[
                "jpg",
                "jpeg",
