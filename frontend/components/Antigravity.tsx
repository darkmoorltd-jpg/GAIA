...
const AntigravityInner = ({
  ...
  const particles = useMemo(() => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      temp.push({ t, factor, speed, xFactor, yFactor, zFactor, mx: x, my: y, mz: z, cx: x, cy: y, cz: z, vx: 0, vy: 0, vz: 0, randomRadiusOffset: (Math.random() - 0.5) * 2 });
    }
    return temp;
  ...
  useFrame(state => {
    ...
    particles.forEach((particle) => {
      ...
      if (dist < magnetRadius) {
        ...
        targetPos.y = projectedTargetY + (ringRadius + wave + deviation) * Math.sin(angle);
      ...
      const distToMouse = Math.sqrt(Math.pow(particle.cx - projectedTargetX, 2) + Math.pow(particle.cy - projectedTargetY, 2));
      let scaleFactor = Math.max(0, Math.min(1, 1 - Math.abs(distToMouse - ringRadius) / 10));
