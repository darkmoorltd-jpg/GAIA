...
st.set_page_config(
    page_title="Extension Dashboard",
    page_icon="🧑‍🌾",
    layout="wide")
...
st.markdown(
    "<h1 style='color:#2e7d32;'>🧑‍🌾 Extension Dashboard</h1>",
    unsafe_allow_html=True)

tab1, tab2, tab3 = st.tabs(
    ["📋 Assign Farmers", "📊 Field Reports", "📈 Performance"])

with tab1:
    st.markdown("### Assign farmers to extension officers")
    # Fetch assignments
    assignments_res = supabase.table(
        "extension_assignments").select("*").execute()
...
with tab2:
    ...
    with st.form("report_form"):
        report_type = st.selectbox(
            "Report Type", [
                "Crop disease", "Pest outbreak", "Soil issue", "General advisory"])
        ...
        if st.form_submit_button("Submit Report"):
            if location and description:
                ...
                st.rerun()
    # View reports
    reports_res = supabase.table("field_reports").select(
        "*").order("created_at", desc=True).limit(50).execute()
    reports = reports_res.data if reports_res.data else []
    if reports:
        for rep in reports:
            st.markdown(
                f"**{rep.get('report_type', '')}** — {rep.get('location', '')}")
...
with tab3:
    ...
    officers = supabase.table("extension_assignments").select(
        "officer_name").execute()
