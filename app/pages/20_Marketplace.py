...
DEMO_LISTINGS = [
    {"id": "1", "crop": "Maize", "variety": "SAMMAZ 15", "quantity": 20, "unit": "tonnes",
     ...
     "description": "High-yield hybrid maize. Drought resistant. Germination 98%."},
    ...
    {"id": "3", "crop": "Beans", "variety": "IT89KD-288", "quantity": 8, "unit": "tonnes",
     ...
     "description": "Organic honey beans. High protein. No pesticides. Export quality."},
    ...
    {"id": "6", "crop": "Yam", "variety": "White Yam", "quantity": 25, "unit": "tonnes",
        ...
        for i, listing in enumerate(filtered):
            with cols[i % 3]:
                ...
                <div class="product-card">
                    ...
                    <div style="font-size:0.9rem;color:#1b5e20;font-weight:600;">📦 {listing['quantity']} {listing['unit']}</div>
                ...
                with col_btn2:
                    if st.button("📞 Contact", key=f"contact_{listing['id']}", use_container_width=True):
                        st.info(f"📞 Contact {listing['farmer']} for pricing and logistics.")
    else:
...
with tab2:
    ...
    demo_orders = [
        {"id": "ORD-001", "item": "Maize (SAMMAZ 15)", "qty": "5 tonnes", "total": "₦1,100,000", "seller": "Ibrahim Musa", "status": "Delivered", "date": "2025-08-10"},
        {"id": "ORD-002", "item": "Rice (FARO 44)", "qty": "3 tonnes", "total": "₦1,050,000", "seller": "Aisha Bello", "status": "In Transit", "date": "2025-08-11"},
        {"id": "ORD-003", "item": "Groundnuts", "qty": "2 tonnes", "total": "₦760,000", "seller": "Usman Sani", "status": "Pending", "date": "2025-08-12"},
    ]
    for o in demo_orders:
        with st.expander(f"📦 {o['id']} — {o['item']} — {o['status']}"):
            c1, c2 = st.columns(2)
            with c1: st.write(f"**Qty:** {o['qty']}"); st.write(f"**Total:** {o['total']}"); st.write(f"**Seller:** {o['seller']}")
            with c2: st.write(f"**Date:** {o['date']}"); st.write(f"**Status:** {o['status']}")
...
with tab3:
    ...
    with st.form("list_produce"):
        c1, c2 = st.columns(2)
        with c1: crop_name = st.text_input("Crop Name *"); variety = st.text_input("Variety")
        with c2: quantity = st.number_input("Quantity *", min_value=1, value=10); price = st.number_input("Price per Unit (₦) *", min_value=1000, value=200000, step=10000)
...
with tab4:
    ...
    c1.metric("Listings", "0"); c2.metric("Sales", "₦0"); c3.metric("Rating", "⭐ New")
...
with st.sidebar:
    ...
    if not st.session_state.cart:
        ...
        for item in st.session_state.cart:
            st.markdown(f"**{item['crop']}** — ₦{item['price']:,}")
            total += item['price']
    ...
    st.caption("🔒 Payment held in escrow until delivery.")
